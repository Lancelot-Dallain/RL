...
class Graphic:
    def __init__(self,file,FOOD_REWARD, POISON_REWARD, DEADLYPOISON_REWARD):
        global posMax
...
class Pacman():
    def __init__(self,file,STEP_REWARD = -1,FOOD_REWARD=10, WIN_REWARD = 50, POISON_REWARD = -20, DEADLYPOISON_REWARD=-100):   
        ...
        #self.state_space =self.grid_size**2 * 2**(len(self.env.food)+len(self.env.poison))        # Nombre total d'états
        self.state_space =self.grid_size**2 * 2**(len(self.env.food))        # Nombre total d'états
        self.reset()
        self.flag=0
        self.verbose=True
    ...
    def state(self):               # retourne l'etat courant
        ...
        flagPoisonEaten= [poison.eat for poison in self.env.poison ]      # recupere liste des flags poisons mangées/non mangées 

        #for i,flag in enumerate(flagFoodEaten+flagPoisonEaten ):
        for i,flag in enumerate(flagFoodEaten ):
    ...
    def init_graph(self,file,STEP_REWARD,FOOD_REWARD,WIN_REWARD,POISON_REWARD,DEADLYPOISON_REWARD):
        ...
        self.win.addshape("images/deadlypoison.gif")
         ...
         #food
        self.env=Graphic(file,FOOD_REWARD, POISON_REWARD,DEADLYPOISON_REWARD)
...
if __name__ == '__main__':
    ...
    env = Pacman('boards/board1.txt')       # Creation de l'environnement
